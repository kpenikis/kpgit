function plotRasters_eachUnit(PlotWhichUnits)
%
...
q = load(fullfile(fn.processed,'Units_AAB055'));
UnitData = q.UnitData;
UnitInfo = q.UnitInfo;
clear q
...
if nargin>0
    theseUnits = find(strcmp({Unit_sort.Session},'QA') & [Unit_sort.Channel]==3);
...
for iUn = theseUnits
    ...
    if minTrs<10
        fprintf('Few trials: skipping ch %i clu %i...\n',Unit_sort(iUn).Channel,Unit_sort(iUn).Clu)
        continue
    end
    ...
    for ist = 1:numel(allStim)
        ...
        if nargin<1 || strcmp(PlotWhichUnits,'each')
            % Set up figure
            hf(ist) = figure;
        ...
        if stid==3 || stid==6
            ...
            TDidx(TDidx==1) = [];
            % Opt: Exclude Pdc trials that follow same ITI rate

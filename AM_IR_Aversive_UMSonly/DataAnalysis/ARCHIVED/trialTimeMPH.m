function trialTimeMPH(PlotThis,select_stim)
%
...
for ir = 1:numel(AMrates)
    hsp(ir)=subplot(numel(AMrates),1,ir); hold on
    plot([yminval ymaxval],[yminval ymaxval],':k'); 
    title(['BMF-fr: ' num2str(AMrates(ir)) ' Hz'])
    if ir==5
        ylabel(PlotThis)
...
%                 mnNtrials = min(cell2mat(cellfun(@(x) size(x,1),MPH.raster,'UniformOutput',false)));
                ...
                isp = ThisResp.(select_stim);
                
                subplot(hsp(isp)); hold on
                
                for stid = unique(MPH.ThisStimID)'
                    
                    thesePds = MPH(MPH.ThisStimID==stid,:);
                    
                    for ipd = 1:size(thesePds,1)
                        
                        plot(thesePds.Starttime(ipd),mean(mean(thesePds.raster{ipd},1))*1000,...
                            'o','MarkerEdgeColor',colors(1+find(AMrates==thesePds.AMrate(ipd)),:))
...
% eps
print_eps_kp(hf,fullfile([savedir '/eps'],[savename '_each']))
print_eps_kp(hfo,fullfile([savedir '/eps'],[savename '_overlay']))
% svg
print_svg_kp(hf,fullfile([savedir '/svg'],[savename '_each']))

...
%     spikes.params.thresh =    [nan];         % for 'manual', set specific thresholds for each channel
    spikes.params.reject = 16;               % for 'auto', set number of standard deviations above background noise       %kp
%     spikes.params.reject =    [nan];         % for 'manual', set specific thresholds for each channel                   %kp
    ...
    % sorting parameters
    spikes.params.refractory_period = 3;    % ms, refractory period (for calculation refractory period violations)
    spikes.params.max_jitter = 0.6;         % ms, width of window used to detect peak after threshold crossing
    spikes.params.agg_cutoff = .05;         %  higher = less aggregation, lower = more aggregation   default: 0.05? 
    ...
    % plot_waveforms
    spikes.params.display.default_waveformmode = 2;  % 1 = show all waveforms, 2 = show 95% waveform bands, 3 = show 2d histogram

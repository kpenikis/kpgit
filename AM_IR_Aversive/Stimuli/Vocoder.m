function [vocoded_x,carriers,envelopes] = Vocoder(x, fs, nchan, cutoff , vocoder_type, verbose)
...
% https://www.mathworks.com/matlabcentral/fileexchange/69403-cochlear-implant-simulation
...
% Apply high-pass pre-emphasis filter   %kp: skip
% pre=0.9378;
% xx=filter([1 -pre], 1, x)';
xx=x;
...
%%% kp: changing envelope extraction to hilbert
 fc=cutoff /(fs/2);
...
for i=1:nchan
    ...
    if(verbose)
        ...
        x_ticks = round(logspace(log10(min_f), log10(fs/2), nchan));
...
% % % % %     envelope=filter(blp,alp,filtwav);
    % kp: instead, extract envelope with hilbert transform + LP
    
    hilbtrans = abs(hilbert(filtwav));
    
    % Lowpass
    lpFilt = designfilt('lowpassiir','FilterOrder',2, ...
        'PassbandFrequency',cutoff,'PassbandRipple',0.2,'SampleRate',fs);
    %         fvtool(lpFilt)
    ...
    voc_band = envelope.*carrier;
    
    % sum bands with equal gain in each channel
    vocoded_x = vocoded_x + voc_band;
end

...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CELLS
whichCells   = 'topEach'; %'SpecRest'; %'Q_pkFR'; %'dpRank_RS'; % pkFR_RS
whatPerc     = 94.5;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TIME
Dur          = 500;
...
%         iC_Spec = find(dpSU(flagRS,ist)>1);
%         iC_Rest = find(dpSU(flagRS,ist)<=1);
...
%             scatter(thisLat,t_up,5*ceil(pk_height),'o','MarkerFaceColor','k','MarkerEdgeColor','none')
            scatter(thisLat,pk_height,ceil(40*dpSU(flagRS(iC_Rest(iu)),ist))+20,'o','MarkerFaceColor','k','MarkerEdgeColor','none')
        ...
        for iu = 1:numel(iC_Spec)
            ...
            subplot(4,4,[9 10 13 14]);
            hold on
%             scatter(thisLat,t_up,5*ceil(pk_height),'o','MarkerFaceColor','r','MarkerEdgeColor','none')
            scatter(thisLat,pk_height,ceil(40*dpSU(flagRS(iC_Spec(iu)),ist))+20,'o','MarkerFaceColor','r','MarkerEdgeColor','none')

        end %iu
        
        subplot(4,4,5:6)
        hold on
        histogram(Result(iC_Spec,2,ist),1:10:500,'FaceColor','r','EdgeColor','none')
        
        subplot(4,4,[11 15])
        hold on
        histogram(Result(iC_Spec,3,ist),1:10:500,'FaceColor','r','EdgeColor','none')
        ...
        subplot(4,4,1:2)
        hold on
        plot(mean(mean(CTTS(iC_Rest,:,:,ist),3,'omitnan'),1)*1000,'k','LineWidth',2)
        plot(mean(mean(CTTS(iC_Spec,:,:,ist),3,'omitnan'),1)*1000,'r','LineWidth',2)
        set(gca,'Color','none')
        xlim([0 500])
        ylim([0 20])
        title('psth')
...
%         end
        ...
        savename = sprintf('%s_%s_stim%i_%s',whichStim,whichCells,ist,'pk_time_height');

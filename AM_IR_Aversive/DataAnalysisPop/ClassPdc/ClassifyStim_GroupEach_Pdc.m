function ClassifyStim_GroupEach
% ClassifyStim_GroupEach
% 
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~
nGrps     =  5;
GrpType   =  'phase'; 'dynRange'; 'tuning'; 'peakFR'; 
Randomize =  1;
%~~~~~~~~~~~~~~~~~~~~~~~~~~
SetNIterations = 500;
%~~~~~~~~~~~~~~~~~~~~~~~~~~
...
% PCMat_it = nan(nStim,nStim,nIterations);\
...
for iGrp = 1:nGroups
    ...
    for iIt = 1:nIterations
        ...
        for ist = 1:nStim
            ...
            T(1,ist) = {mean(Group_Time_Trial_Stim(iGrp,:,temp_trs,ist),3)};
        ...
        for isTrue = 1:nStim
            ...
            for it = test_trs
                ...
                % Concatenate data from each group
                S = Group_Time_Trial_Stim(iGrp,:,it,isTrue);
...
%                 [blockAssignment,maxR] = rc_calcR(T,S);      %max rcorr
    ...
    PCMat(:,:,iGrp) = mean(percentCorrectMat,3,'omitnan');
    
    thisPC = PCMat(:,:,iGrp);
    
    % Extract statistics from confusion matrix
    meanPC = mean(diag(thisPC));
    dprime = norminv(mean(diag(thisPC)),0,1) - norminv(mean(thisPC( ~diag(diag(thisPC)) )),0,1); % ?
    ...
    ylabel('True stim')
    xlabel('Assigned')
    title(sprintf('%s %i  |  PC=%0.2f, d''=%0.2f', GrpType,iGrp,meanPC,dprime)) 
    
    
    %% Save figure
    
    savename = sprintf('PCMat_%s-grp%i_%ims_tempsz%i',GrpType,iGrp,binsize,TempSize);
...
savename = sprintf('GrpEach_%s_%ims_tempsz%i',GrpType,binsize,TempSize);

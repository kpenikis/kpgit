function ClassifyStimulus_nSpkVec
% ClassifyStimulus_SU
%
...
%     percentAssignment = nan(nStim,nStim,nTrials);
    ...
    for iIt = 1:nTrials
        ...
        T = cell(2,nStim);
        for ist = 1:nStim
            T(1,ist) = {permute(sum(sum(Cell_Time_Trial_Stim(:,1:StimDur,iIt,ist),2)),[1,4,2,3])'};
            T(2,ist) = {iIt};
        end
        ...
        for isTrue = 1:nStim
            ...
            for it = test_trials
                ...
                S = sum(sum(Cell_Time_Trial_Stim(:,1:StimDur,it,isTrue),2))';
...
savename = ['PCMat_nSpk_Pop' app_str '_' num2str(StimDur) 'ms'];

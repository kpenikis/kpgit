...
close all
global fn nIterations StimDur exclOnset TempSize
...
%         foo = conv(sum(Cell_Time_Trial_Stim(:,:,iIt,ist),1),convwin);
...
%             foo = conv(sum(Cell_Time_Trial_Stim(:,:,it,isTrue),1),convwin);
%             S = foo(floor(numel(convwin)/2)+(0:StimDur-1));
            S = sum(Cell_Time_Trial_Stim(:,:,it,isTrue),1);
%             S = Cell_Time_Trial_Stim(:,:,it,isTrue)';
...
savename = ['PCMat_tsPopAvg_conv0_Ed' app_str '_' num2str(StimDur) 'ms'];

...
%    table. 
%   
%    DOES NOT YET WORK WITH KiloSort DATA.
%
%  KP, 2018-07, updated 2019-02
%
...
alt_color = [50  66  69]./255;

% Set colors
colors = [ stim_colors(stimID,:); [1 1 1] ];

raster_colors = [  0  92 162 ;   0   0   0]./255;
bkgrnd_colors = [255 255 255 ; 230 231 232]./255;
...
% Set up figure
hf = figure;
set(gcf,'Position',0.8*halfscreen.* [1 1 figwidthscales(stimID) 1])
hold on
hs(1,1) = subplot(9,1,1);
set(gca,'xlim',[0 figwidthscales(stimID)*1000],'xtick',[],'ytick',[])
hs(1,2) = subplot(9,1,2:9);
set(gca,'xlim',[0 figwidthscales(stimID)*1000],'xtick',[],'ytick',[])
...
for iUn = theseUnits
    ...
    if numel(raster_x)>0
        ...
        fill([0 Duration Duration 0],add_trs + [0 0 max(raster_y)+1 max(raster_y)+1],bkgrnd_colors(1+mod(N,size(bkgrnd_colors,1)),:),'EdgeColor','none')
        plot(raster_x, raster_y + add_trs,...
            '.','MarkerSize',rasterdotsize,'Color',raster_colors(1+mod(N,size(raster_colors,1)),:))
%         plot(raster_x, raster_y + add_trs,...
%             '.','MarkerSize',rasterdotsize,'Color',colors(mod(N,size(colors,1))+1,:))
        
        add_trs = add_trs + max(raster_y)+1;
...
%     subplot(hs(ist,3)); hold on
    ...
    PSTH_agg(N,:)   = mean(psth,1,'omitnan');
    PSTH_z_agg(N,:) = mean(psth_z,1,'omitnan');
...
savename = sprintf('AllUnits_%s_contrast',Info.stim_ID_key{stimID});

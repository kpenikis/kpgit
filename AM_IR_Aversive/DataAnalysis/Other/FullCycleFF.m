function FullCycleFF( UnitData, UnitInfo )
...
for iUn = 1:numel(UnitData)
    ...
    FF      = nan(size(Wins,1),numel(AMrates),3);
    
    for irate = AMrates(1:3)
        ...
        for iw = 1:size(Wins,1)
            ...
            raster   =  cell2mat(MPH(idx_pdc,:).raster);
            OWRatio(iw,irate==AMrates,1)  =  ( mean(mean(raster(:,Window),1)) - mean(mean(raster,1)) ) / mean(mean(raster,1));
            FF(iw,irate==AMrates,1)       =    std(mean(raster(:,Window),2),1) / mean(mean(raster(:,Window),2),1);
              var(mean(raster(:,Window),2),1) / mean(mean(raster(:,Window),2),1);
...
%             keyboard
        ...
        polarplot([Theta; Theta(1)],[FF(:,irate==AMrates,1); FF(1,irate==AMrates,1)],'-','Color',thiscolor)

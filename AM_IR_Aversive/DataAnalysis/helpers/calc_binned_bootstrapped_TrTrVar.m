function TrialTrialVariance = calc_binned_bootstrapped_TrTrVar(full_raster,binsize)
...
nIterations = 50*size(raster,1);
TrTrVar = nan(nIterations,1);
for iteration = 1:nIterations
    
    trials = randperm(size(raster,1));
    
    TrTrVar(iteration) = mean(var(raster_bin(trials(1:nTrs),:),1));
    
end


TrialTrialVariance = mean(TrTrVar);

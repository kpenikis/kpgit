...
if nargin>0 && exist('select_subject','var')
    ...
    subjects = { 'AAB_265058' }; % 'AAB_265059' 'WWWf_253400' 'WWWlf_253395'  };
end
    ...
    for ich = 1:length(Channels)
        ...
        if isempty(clus)
            ...
            for iclu = 1:numel(clus)
                ...
                clu        = clus(iclu);
                spiketimes = unique(round(Spikes.sorted(Channels(ich)).spiketimes(Spikes.sorted(Channels(ich)).assigns==clu') * 1000));  %ms
...
elseif exist('Clusters','var') && ~exist('Spikes','var')      % >>> KS <<<
    ...
    for iclu = 1:numel(Clusters)
        ...
        spiketimes = unique(round(Clusters(iclu).spikeTimes * 1000))'; %ms

function analyzeMPH_withinContext( PlotThis, select_stim, UnitFilt )
%
...
if nargin<1
    PlotThis = 'covSpk'; 'FR'; 'FF'; 'TrV'; 'VS'; 'MP'; 'CrSc'; 'Rel'; 'CV'; 'r_all'; 'r_pdc';
    select_stim = 'iBMF_FR'; 'iBMF_VS'; 'iSync'; 'all';
    UnitFilt =    'All';  'Sparse';  'HighFR';
elseif nargin==1
    select_stim = 'iBMF_FR'; 'iBMF_VS'; 'iSync'; 'all';
    UnitFilt =    'All';  'Sparse';  'HighFR';
elseif nargin==2
...
if AMrespFilt %skip sessions with no Robust SUs
    ...
    % be flexibly changed.
    [sig_ids,SessResp] = identifyResponsiveUnits_old(SessResp);
    
    if strcmp(UnitFilt,'NonRespUn')
        NR_ids = 1:numel(SessResp);
        NR_ids(sig_ids)=[];
        Channels = unique([SessResp(NR_ids).Channel]);
    else
%         Channels = unique([SessResp.Channel]);
...
for channel = Channels
    ...
    for clu = Clus'
        ...
        switch UnitFilt
            case 'Sparse'
                if ThisResp.Sparse==0, continue, end
            case 'HighFR'
                if ThisResp.Sparse==1, continue, end
        end
        
        if strcmp(select_stim,'iBMF_FR') && (~isfield(ThisResp,'iBMF_FR') || isempty(ThisResp.iBMF_FR))
            continue 
        end
        if strcmp(select_stim,'iBMF_VS') && (~isfield(ThisResp,'iBMF_VS') || isempty(ThisResp.iBMF_VS))
            continue
        end
        ...
        spiketimes = spiketimes-round(ThisResp.IntTime);
        ...
        for spl = dBSPL
            for lpn = LP
                ...
                %>>>>>>>>>>>>>>>>>>>>>>>>|||<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                %>>>>>>>>>>>>>>>>>>>>>>>>|||<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                % Create table with all unique periods in it
                MPH = makeMPHtable(TrialData,Info.artifact(channel).trials,spl,lpn,spiketimes);
                %>>>>>>>>>>>>>>>>>>>>>>>>|||<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                %>>>>>>>>>>>>>>>>>>>>>>>>|||<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
% Pdc-Pdc
[r,p_r] = corrcoef( Pdcmat(:,1), Pdcmat(:,2) );
...
% IR-IR 
[r,p_r] = corrcoef( PdIRmat(:,2), PdIRmat(:,1) );
...
%% SAVE PLOTS AND DATA
% UnitFilt
savedir = fullfile(fn.processed,'MPHmetrics',UnitFilt);

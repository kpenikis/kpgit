function [rez, DATA, uproj] = preprocessData_kp(ops)
tic;
uproj = [];
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% OPEN FILE TO WRITE FILTERED DATA     (kp)
samps_stored = [];
fidFilt    = fopen([strtok(ops.fbinary,'.') '_filt.dat'], 'w');
ops.fbinary_filt = [strtok(ops.fbinary,'.') '_filt.dat'];
...
h=waitbar(0,'Computing whitening filters');
...
while 1
    ...
    waitbar(ibatch/Nbatch,h);
    
    nsampcurr = size(buff,2);
    if nsampcurr<NTbuff
        buff(:, nsampcurr+1:NTbuff) = repmat(buff(:,nsampcurr), 1, NTbuff-nsampcurr); 
        padsamps = NT-nsampcurr-ops.ntbuff;
    end
    beg_batch = [beg_batch; (NT-ops.ntbuff)*(ibatch-1)+[0 NT-ops.ntbuff-1-padsamps]];
    ...
    % COMMON AVERAGE SUBTRACTION    (kp)
    for ich = 1:length(chanMapConn)
        datr(:,chanMapConn(chanMapConn==chanMapConn(ich))) = datr(:,chanMapConn(chanMapConn==chanMapConn(ich))) - median(datr(:,chanMapConn(chanMapConn~=chanMapConn(ich))),2);
    end
    
    % WRITE FILTERED DATA TO FILE   (kp)
    if ibatch<=Nbatch_buff
        samps_stored = [samps_stored; ((NT-ops.ntbuff)*(ibatch-1) - 2*ops.ntbuff) + (ioffset + [ops.ntbuff+1 NT-padsamps])];
        fwrite(fidFilt, int16( datr(ioffset + ((ops.ntbuff+1):(NT-padsamps)),:)' ), 'int16');
    end
    
    datr = datr(:, chanMapConn);

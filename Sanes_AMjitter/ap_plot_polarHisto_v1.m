function ap_plot_polarHisto_v1(subject,session,channel,clu,raster)
...
for ks = 1:numel(raster)
    ...
    nt = max(data.y);
    sp_phase_counts = zeros(numel(tVec)-1,nbins);
    sp_phase_avg = zeros(nt,nbins);
    ...
    savename = sprintf('%s_%s_polarHisto_ch%i_clu%i_AM4Hz_jitter%s_blk%i',subject,session,channel,clu,data.jitter,block);

...
%         end
    ...
    set(hFs,'Position',[1 scrsz(4)/2 scrsz(3) scrsz(4)/2],...
        'Nextplot','add');
    fill([1:length(stim) length(stim):-1:1] /fs*1000,[stim fliplr(-stim)],[0 0 0])
    hold on
    xtx = 500:500:3000;
    for itx=xtx
%         plot([itx itx],[-2 2],'k--')
...
%     title(['Jitter range: ' num2str(min(buffer)) ' to ' num2str(max(buffer)) ' Hz '])
    ...
    % Save figure
    set(hFs,'PaperOrientation','landscape');
%     print(hFs,'-dpdf', '/Users/kpenikis/Documents/MATLAB/Sanes_AMjitter/stimuli/201612/example_stimulus','-bestfit')
    print(hFs,'-depsc', '/Users/kpenikis/Documents/MATLAB/Sanes_AMjitter/stimuli/201612/example_sAM')

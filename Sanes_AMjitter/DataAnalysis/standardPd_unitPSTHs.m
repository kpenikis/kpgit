function standardPd_unitPSTHs(subject)
...
for isess = sessions'
    for iu = unique(InfoTable(strcmp(InfoTable.Session,isess{:}),:).cluname,'stable')'
        ...
        for iiv = unique(InfoTable(strcmp(InfoTable.Session,isess{:}) & strcmp(InfoTable.cluname,iu{:}),:).DiscrmVar,'stable')'
            ...
            pars = unique([filtInfo.HP filtInfo.LP filtInfo.dBSPL filtInfo.CenterRate],'rows');
            ...
            for ip = 1:size(pars,1)
                ...
                for ib = behavs'
                    ...
                    % Load Data struct
%                     fnData = sprintf('%s_sess-%s_Data.mat',subject,isess{:});
%                     load(fullfile(fn.processed,subject,fnData))
%                     Data.(iu{:}).stimdata
%                     
%                     
%                     
                    ...
                    if nTrials_pdc ~= size(resp_pdc,1)
                        keyboard
                    end
                    
                    
                    
                    %% Make raster/histo of periodic only, with all relevant stats printed.
                    
                    %seperate function
%                     plot_rasterHisto(resp_pdc,UnitData)
%                     resp_pdc
%                     FRsmooth_pdc
%                     UnitData.VS
%                     UnitData.Corr0
...
%                     
                    ...
                    %% Plot PSTH of 2nd, 4th, 6th periodic periods

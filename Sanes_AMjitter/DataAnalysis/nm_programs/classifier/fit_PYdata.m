...
%
%Written by ML Caras, updated by KP 03-2017.



% Because we want the threshold to be determined by a certain dprime value,
% convert this to PC thresh and change in options struct
n_fa = data_to_fit(1,2);
n_nogo = data_to_fit(1,3);
zFA = sqrt(2)*erfinv(2*(n_fa/n_nogo)-1);
...
results = rmfield(results,{'X1D' 'marginals' 'marginalsX' 'marginalsW' 'weight' 'Posterior'});
